...
# PAGE TITLE
# ------------------------------------------------
st.markdown('<div class="main-title">Brayton Cycle Simulation</div>', unsafe_allow_html=True)
st.markdown("Simulate an ideal air-standard Brayton thermodynamic cycle.")
st.markdown("---")
...
with col1:
    T1 = st.number_input("Initial Temperature T1 (K)",
                          min_value=200, max_value=1000, value=300, step=1)
    rp = st.number_input("Pressure Ratio rp",
                          min_value=2.0, max_value=30.0, value=8.0, step=0.1,
                          format="%.1f")

with col2:
    T3 = st.number_input("Maximum Temperature T3 (K)",
                          min_value=500, max_value=2500, value=1400, step=10)
...
<div class="flow-card">
  ...
  <div class="flow-grid">
    ...
    <div class="flow-step"><span>4 → 1</span> Constant-Pressure Heat Rejection</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
# RUN BUTTON
# ------------------------------------------------
run = st.button("Run Brayton Simulation", type="primary")
st.markdown("---")

if run:
    results = brayton_cycle(T1, T3, rp, V1=V1_m3)
    ...
    P1c, P2, P3, P4   = results["P"]
    V1c, V2, V3, V4   = results["V"]
    T1c, T2, T3c, T4  = results["T"]
    S1c, S2, S3, S4   = results["S"]
    ...
    <div class="charts-grid">
      <div class="chart-card">
        ...
        <div class="legend">
          ...
          <div class="legend-row"><div class="legend-line" style="background:#e05c3a"></div>2→3 Constant-pressure heat addition</div>
          <div class="legend-row"><div class="legend-line" style="background:#4aad6e"></div>3→4 Isentropic expansion</div>
          <div class="legend-row"><div class="legend-line" style="background:#c8941a"></div>4→1 Constant-pressure heat rejection</div>
        </div>
      </div>
      <div class="chart-card">
    ...
    const cp    = {cp_val};
    const P1={P1c}, P2={P2}, P3={P3}, P4={P4};
    const V1={V1c}, V2={V2}, V3={V3}, V4={V4};
    const T1={T1c}, T2={T2}, T3={T3c}, T4={T4};
    const S1={S1c}, S2={S2}, S3={S3}, S4={S4};
    ...
    (function() {{
      ...
      const seg23 = Array.from({{length: steps+1}}, (_, i) => [V2 + i*(V3-V2)/steps, P2]);
      const seg34 = Array.from({{length: steps+1}}, (_, i) => {{
        const v = V3 + i * (V4 - V3) / steps;
        return [v, P3 * Math.pow(V3 / v, gamma)];
      }});
      const seg41 = Array.from({{length: steps+1}}, (_, i) => [V4 + i*(V1-V4)/steps, P1]);

      const allV = [V1, V2, V3, V4];
      const allP = [P1, P2, P3, P4];
      const Vmin = Math.min(...allV) * 0.9,  Vmax = Math.max(...allV) * 1.05;
      const Pmin = Math.min(...allP) * 0.82, Pmax = Math.max(...allP) * 1.08;
      ...
      curve(ctx, seg34, '#4aad6e', toX, toY);
      curve(ctx, seg41, '#c8941a', toX, toY);
    ...
    (function() {{
      ...
      const seg34 = Array.from({{length: steps+1}}, (_, i) => [S3, T3 + i*(T4-T3)/steps]);
      const seg41 = Array.from({{length: steps+1}}, (_, i) => {{
        const t = T4 + i*(T1-T4)/steps;
        return [S4 + cp * Math.log(t / T4), t];
      }});
      ...
      curve(ctx, seg34, '#4aad6e', toX, toY);
      curve(ctx, seg41, '#c8941a', toX, toY);
      [[S1,T1,'1'],[S2,T2,'2'],[S3,T3,'3'],[S4,T4,'4']].forEach(([s,t,l]) => dot(ctx,s,t,l,toX,toY));
    ...
    st.markdown("Variation of Brayton cycle thermal efficiency with pressure ratio.")

    graph_col, info_col = st.columns(2)

    with graph_col:
        gamma_val = results["gamma"]
        current_rp = results["rp"]

        html_analysis_code = f"""
        <style>
          * {{ box-sizing: border-box; margin: 0; padding: 0; }}
          body {{ background: transparent; font-family: sans-serif; }}
          .chart-card {{
        ...
        <div class="chart-card">
          <div class="chart-title">📊 Efficiency vs. Pressure Ratio</div>
          <canvas id="eff-canvas"></canvas>
        </div>

        <script>
        const gamma = {gamma_val};
        const currentRp = {current_rp};

        const k = (gamma - 1) / gamma;
        const pts = [];
        for (let rp = 2; rp <= 25; rp += 0.2) {{
          let eta = (1 - (1 / Math.pow(rp, k))) * 100;
          pts.push([rp, eta]);
        }}

        const c = document.getElementById('eff-canvas');
        const dpr = window.devicePixelRatio || 1;
        const w = c.parentElement.clientWidth - 32;
        const h = 220; 
        ...
        const RpMin = 2, RpMax = 25;
        const EtaMin = 10, EtaMax = 70;

        const toX = rp => pad.l + (rp - RpMin) / (RpMax - RpMin) * (w - pad.l - pad.r);
        const toY = eta => h - pad.b - (eta - EtaMin) / (EtaMax - EtaMin) * (h - pad.t - pad.b);

        // 1. Grid
        ctx.strokeStyle = 'rgba(255,255,255,0.06)';
        ctx.lineWidth = 0.5;
        for (let r = 5; r <= 25; r += 5) {{ ctx.beginPath(); ctx.moveTo(toX(r), pad.t); ctx.lineTo(toX(r), h - pad.b); ctx.stroke(); }}
        for (let e = 20; e <= 60; e += 10) {{ ctx.beginPath(); ctx.moveTo(pad.l, toY(e)); ctx.lineTo(w - pad.r, toY(e)); ctx.stroke(); }}
        ...
        ctx.font = '10px sans-serif';
        ctx.textAlign = 'center';
        [2, 5, 10, 15, 20, 25].forEach(r => ctx.fillText(r, toX(r), h - pad.b + 14));
        ctx.textAlign = 'right';
        [20, 30, 40, 50, 60].forEach(e => ctx.fillText(e + '%', pad.l - 8, toY(e) + 4));

        ctx.fillStyle = '#bbb';
        ctx.font = '11px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('Pressure Ratio (rp)', pad.l + (w - pad.l - pad.r) / 2, h - 2);
        ...
        // 4. Highlight
        if (currentRp >= RpMin && currentRp <= RpMax) {{
          const currentEta = (1 - (1 / Math.pow(currentRp, k))) * 100;
          const cx = toX(currentRp);
          const cy = toY(currentEta);
          ctx.strokeStyle = 'rgba(224, 92, 58, 0.4)';
          ctx.lineWidth = 1;
          ctx.setLineDash([4, 4]);
          ctx.beginPath(); ctx.moveTo(cx, h - pad.b); ctx.lineTo(cx, cy); ctx.stroke();
          ctx.beginPath(); ctx.moveTo(pad.l, cy); ctx.lineTo(cx, cy); ctx.stroke();
          ctx.setLineDash([]);
          ctx.beginPath(); ctx.arc(cx, cy, 5, 0, Math.PI * 2);
          ctx.fillStyle = '#1a1a2e'; ctx.fill();
          ctx.strokeStyle = '#e05c3a'; ctx.lineWidth = 2; ctx.stroke();
          ctx.fillStyle = '#fff';
          ctx.font = 'bold 10px sans-serif';
          ctx.textAlign = 'left';
          ctx.fillText(' Op. Point (rp=' + currentRp.toFixed(1) + ')', cx + 8, cy + 3);
        }}
        </script>
        """

        components.html(html_analysis_code, height=270)
