...
st.markdown('<div class="main-title">Otto Cycle Simulation</div>', unsafe_allow_html=True)
st.markdown("Simulate an ideal air-standard Otto thermodynamic cycle.")
st.markdown("---")
...
with col1:
    ...
    T3 = st.number_input("Peak Temperature T3 (K)",
                          min_value=500, max_value=3000, value=1500, step=1)

with col2:
    r  = st.number_input("Compression Ratio r",
                          min_value=2.0, max_value=20.0, value=8.0, step=0.1,
                          format="%.1f")
...
if T3 <= T1:
    st.error("Peak temperature T3 must be greater than initial temperature T1.")
    st.stop()

# ------------------------------------------------
# RUN BUTTON & FLOWSHEET PANEL
# ------------------------------------------------

run = st.button("Run Otto Simulation", type="primary")
...
if run:
    ...
    results = otto_cycle(T1, r, T3, V1=V1_m3)
    ...
    df = pd.DataFrame({
        "State":      ["1 — Start of compression",
                       ...
                       "3 — After constant-vol. heat addition",
                       "4 — After isentropic expansion"],
        "T (K)":      [f"{t:.1f}" for t in T],
        "P (kPa)":    [f"{p:.2f}" for p in P],
    ...
    <div class="charts-grid">
      <div class="chart-card">
        ...
        <div class="legend">
          ...
          <div class="legend-row"><div class="legend-line" style="background:#c8941a"></div>4→1 Constant-volume heat rejection</div>
        </div>
      </div>
      <div class="chart-card">
    ...
    // Convert volumes to Liters directly for flawless graph scaling
    const V1={V1c} * 1000, V2={V2} * 1000, V3={V3} * 1000, V4={V4} * 1000;
    ...
    (function() {{
      ...
      const seg41 = Array.from({{length: steps+1}}, (_, i) => [V4, P4 + i * (P1 - P4) / steps]);

      const Vmin = V2 * 0.85,  Vmax = V1 * 1.05;
      const Pmin = P1 * 0.80,  Pmax = P3 * 1.05;
      ...
      drawAxes(ctx, w, h, pad, 'Volume (L)', 'Pressure (kPa)', xTicks, yTicks, toX, toY,
        x => x.toFixed(2) + 'L', y => y.toFixed(0));
    }})();
